...
function PolinomioTaylor1{T<:Real}(g::Int64,CX::Array{TaylorN{T}},CY::Array{TaylorN{T}})
    ...
    =#
    #@show("entro a la funcion PolinomioTaylor1")
    ...
    if g == 1
        Taylor = [Taylor1([x], g+10),Taylor1([y], g+10)]
        ...
        Taylor = [Taylor1(LX, g+10), Taylor1(LY, g+10)]
...
"""
#function InvarianciaR{T<:BigFloat}(λ_v, g, CX, CY)
#function InvarianciaR{T<:Float64}(λ::Float64,Polinomio::Array{Taylor1{TaylorN{T}},1})
...
function InvarianciaR(λ_v, g, CX, CY,λ,Polinomio)
    ...
    #=************************************MODIFICADA
    CXn = [CX[i] for i in 1:g+1]
    CYn = [CY[i] for i in 1:g+1]

    #***************************MODIFICADA

    xλt = Taylor1(λ_v[1].*CXn, g+10)
    yλt = Taylor1(λ_v[2].*CYn, g+10)
    =#
    xλt = Taylor1(λ_v[1].*CX, g+10)
    yλt = Taylor1(λ_v[2].*CY, g+10)

    @show("g2")



    #@show("LLego hasta antes de evaluar")

    variable_t = Taylor1([0.,1.])
    @show("g3")
    #@show("Llego hasta doonde evaluo")
    #por alguna razón esto no le gusta ... aunque parece ser la misma cosa de arriba
    #=
    ...
    #@show(yλt)
    #@show(typeof(yλt))
    #para verificar que sean parecidas..
    #resta  = yλt1-yλt
    #@show(resta)
...
function InvarianciaR{T<:BigFloat}(λ::BigFloat,Polinomio::Array{Taylor1{TaylorN{T}},1})
    # El arreglo λ_v contiene las potencias del valor propio en orden .
    # Es importante hacer la distinción de una lista para X y otra para Y puesto que dependiendo del
    # punto fijo donde se esté calculando
    # el primer valor de λ en X será diferente del primer valor de λ en Y
    @show("Llego hasta antes de evalurar")
    variable_t= Taylor1([big(0.),big(1.)])
    @show(variable_t)
    xλt = Polinomio[1](λ*variable_t)
    @show(xxλt)
    yλt = Polinomio[2](λ*variable_t)
    @show("llego hasta evaluar")
...
function EigenValores{T<:Float64}(M::Array{T,2})
    ...
    if ValsOrden_aux[1] > ValsOrden_aux[2]
        ValsOrden = [ValsOrden_aux[2],ValsOrden[1]]
...
function Variedades{T<:Real}(Mapeo, orden::Int64, PuntoFijo::Array{T,1},tipo_v) #, k::T, l::T)
    ...
    for g in 2:orden
            ...
            #@show(CX)
            #@show(CY)
            #@show("antes de entrar a polinomio T")
            #***********************************MODIFICADOOO
            @show("a")
            ...
            #@show(t)
            #@show(λ)
            #@show("Llego hasta entes de evaluar la función")
            ...
            @show("k")
            #calculamos el término independiente en la ecuación
            ...
            @show(vec_orden_g)
            @show(typeof(vec_orden_g))
            @show(length(vec_orden_g))
...
"""
function Invariancia{T<:Real}(Mapeo, Pol_vec::Array{Taylor1{T},1}, λvec::Array{Taylor1{T},1} ,modulo::Real)
...
function Error3{T<:Real}(PuntoFijo::Array{T,1},Taylor::Array{Taylor1{T}},eigval, eigvec,Tiempo::Array{T}, paso::Real,orden::Real)
    #@show("entro a la funcion")

    var_aux = Taylor1([0.,1.])
    U = eigvec
    cero = Taylor1([0.])
    #matriz_a = [exp(eigval[1]*var_aux) cero ; cero  exp(eigval[2]*var_aux)]
    prim = 1-eigval[1]
    cuar = 1+1.5-eigval[1]
    ...
    #Invariancia_O = transpose(Taylor)-h
    Invariancia_O =[abs(Taylor[1](0.))^orden,abs(Taylor[2](0.))^orden]
#=
    Val=T[]
    Tiem=T[]
    for t = Tiempo[1]:paso:Tiempo[2]
        x = evaluate(Invariancia_O[1], t)
        y = evaluate(Invariancia_O[2], t)
        norma = norm([x,y], Inf)
        push!(Val, norma)
        push!(Tiem, t)
...
#function MetParametrización{T<:Real}(Mapeo ,modulo::Real, orden::Int64, PuntoFijo::Array{T}, Tiempo::Array{T}, paso::Real)
function Inestable{T<:Real}(Mapeo , orden::Int64, PuntoFijo::Array{T}, Tiempo::Array{T}, paso::Real)
...
#..............................................................................................................................................
function Estable{T<:Real}(Mapeo, orden::Int64, PuntoFijo::Array{T}, Tiempo::Array{T}, paso::Real)
